...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    private AccountInfo getAccountInfo(String email, String role, HttpServletResponse response) throws IOException {
        if ("USER".equals(role)) {
            return checkAccount(
                    userRepository.findByEmail(email).orElseThrow(() -> new ResourceNotFoundException(AppConstants.USER_NOT_FOUND)),
                    userDetailsService::loadUserByUsername,
                    AppConstants.USER_ACCOUNT_INACTIVE,
                    AppConstants.USER_ACCOUNT_DELETED,
                    response
            );
        } else {
            return checkAccount(
                    adminRepository.findByEmail(email).orElseThrow(() -> new ResourceNotFoundException(AppConstants.ADMIN_NOT_FOUND)),
                    adminDetailsService::loadUserByUsername,
                    AppConstants.ADMIN_ACCOUNT_INACTIVE,
                    AppConstants.ADMIN_ACCOUNT_DELETED,
                    response
            );
        }
    }

    private <T> AccountInfo checkAccount(T account,
                                         java.util.function.Function<String, UserDetails> loadUserDetails,
